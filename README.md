...
o	Example: Time Series Forecasting Comparison: Seq2Seq Attention vs. Baseline LSTM
...
List the main files so a reviewer knows where to look.
File	Description
data_preprocessing.ipynb	Scripts for data loading, scaling, and generating time sequences ($\mathbf{X}$, $\mathbf{Y}$).
seq2seq_attention.py	Python script defining the Attention-based model architecture.
baseline_lstm.py	Python script defining the Baseline Stacked LSTM model.
model_training.ipynb	Notebook containing all training loops, stability fixes, and final evaluation metrics.
Results/	Directory containing the final saved model (Baseline_LSTM_Best.keras) and metric logs.
